...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication)
        ...
        final String email = authentication.getPrincipal()
                .toString();//Trả về thông tin đại diện cho người dùng.(username/ obj UserDetails)
        final String password = authentication.getCredentials()
                .toString();//Trả về thông tin xác thực(password)
    ...
    private List<SimpleGrantedAuthority> getRoles(User user) {
        ...
        if (rolesAuthorities.stream()
            ...
            Set<Long> roleIds = roleEntities.stream().map(roleEntity -> roleEntity.getId())
                    .collect(Collectors.toSet());
